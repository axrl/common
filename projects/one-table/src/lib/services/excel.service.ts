...
export class ExcelService {
    ...
    create<T>(data: T[], filename: string, keys: {[key: string]: string}) {
        ...
        ws['!cols'] = objectKeys(keys).map((key, index) => {
    ...
    translateAndCreate<T extends {}>(data: T[], filename: string) {
        return this.translationService.translations$.pipe(
            map(translations => {
                ...
                this.create(
                    data.map(item =>
                        objectKeys(item).reduce((acc: Record<string, string>, key: keyof T) => {
                            if (key !== 'interfaceType' && !(item[key] instanceof Observable)) {
                                const newKey = this.translationService.translate(
                                    translations,
                                    String(key),
                                );
                                keys[newKey] = String(key);
                                acc[newKey] =
                                    isValue(item[key]) && typeof item[key] === 'object'
